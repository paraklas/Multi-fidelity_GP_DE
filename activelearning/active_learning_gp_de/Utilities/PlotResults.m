...
if dim == 1
    ...
    clf
    hold
    h(1) = plot(Xts, Exact_rhs,'b','LineWidth',3);
    h(2) = plot(Xts,Kpred_rhs,'r--','LineWidth',3);
    [l,h(3)] = boundedline(Xts, Kpred_rhs, 2.0*sqrt(Kvar_rhs), ':', 'alpha','cmap', color2);
    outlinebounds(l,h(3));
    
    h(4) = plot(Xts,Kpred_rhs_L,'m--','LineWidth',2);
    [l,h(5)] = boundedline(Xts, Kpred_rhs_L, 2.0*sqrt(Kvar_rhs_L), ':', 'alpha','cmap', color2);
    outlinebounds(l,h(5));
    ...
    clf
    hold
    h(1) = plot(Exact_rhs_L, Exact_rhs,'b','LineWidth',3);
    xlabel('$f_{1}(x)$');
    ylabel('$f_{2}(x)$');
