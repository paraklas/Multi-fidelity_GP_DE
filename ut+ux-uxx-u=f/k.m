...
%     K = G.* (  bsxfun(kt,t,s') + bsxfun(kx,x,y'));
    ...
    K = G.*(bsxfun(kt,t,s') + bsxfun(kx,x,y') + bsxfun(ktt,t,s').*bsxfun(kxx,x,y'));
...
elseif i==1
    ...
    K = exp(-2*(logthetat + 2*logthetax))*( C +  bsxfun(kt,t,s') + bsxfun(kx,x,y')   );
    K = G.*K;
...
elseif i == 2
    ...
    K = 0.5*exp(-2*(logthetat + 2*logthetax))* ( bsxfun(kt,t,s') + C1 + C2 + C3 + C4 + C5     );
...
elseif i==3
    ...
    kt = @(t,s) (s-t);
    kx = @(x,y) (x-y);
    K = (1/2).*exp(1).^((-2).*logthetat+(-5).*logthetax).*(exp(1).^(logthetat+3.*logthetax).*((-1).*exp(1).^logthetat.*(12+5.*bsxfun(kx,x,y').^2)+2.*bsxfun(kt,t,s').*bsxfun(kx,x,y').^3)+exp(1).^(4.*logthetax).*(exp(1).^(2.*logthetat).*(2+bsxfun(kx,x,y').^2)+(-1).*bsxfun(kt,t,s').^2.*bsxfun(kx,x,y').^2+exp(1).^logthetat.*bsxfun(kx,x,y').*((-4).*bsxfun(kt,t,s')+ bsxfun(kx,x,y')))+exp(1).^(2.*(logthetat+logthetax)).*(39+bsxfun(kx,x,y').^2).*bsxfun(kx,x,y').^2+(-14).*exp(1).^(2.*logthetat+logthetax).*bsxfun(kx,x,y').^4+exp(1).^(2.*logthetat).*bsxfun(kx,x,y').^6);
